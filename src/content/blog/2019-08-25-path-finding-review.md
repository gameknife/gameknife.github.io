...
date: 2019-08-25
category: tech
description: "在新地方, 第一个需要处理的任务就是寻路. 说起来, 跟我做的独立项目有一些类似, 场景都是动态的. 需要动态的碰撞检测, 动态的寻路."
tags: ["算法", "游戏ai"]
legacyUrl: "/tech/2019/08/25/path-finding-review/"
---
...
俗称贪心算法, 从起点到终点, 每次都从临近的可行走节点, 选择一个当前的"最佳"解, 一般是欧式空间里, 距离终点最近的那一个节点. 逐步往下, 不走回头路.
...
因此,A*搜索在Dijkstra的基础上, 加入了一个h(n), 使得搜索可以往"更为可行"的方向进行,而放弃那些不太有可能性的方向,此为"启发"的含义

可以看到

* 如果将cost函数变为f(n) = g(n), 那A*算法便退化为Dijkstra算法, 为一个全量搜索
...
对node进行连通性梳理, 传统的2d grid, 连通性就是4或8方向. 而为了能够达到各方向行走, 我们可以在2层-3层空间内进行连通

##### 问题

对于多层的可行走区域, 暂时没有很好的想法, 目前可以考虑将空间分为多层2d空间, 在部分地方, 进行跨空间的连通
...
这样的算法足够简单,也足够收敛.
并且,在构建连通性时,可以直接将每一个step的g(n)缓存在节点上,更加高效
