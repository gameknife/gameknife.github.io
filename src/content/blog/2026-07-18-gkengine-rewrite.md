...
> 说明一下：这篇的内容和判断都是我自己的，AI 帮我理了理结构和文字。后面应该都会这么写。

先看两张图。

![gkEngine：D3D9 延迟光照，HUD 上还写着 RendererD3D9。先看那颗金球。](../../assets/blog/gkengine-conf-room.webp)

![gkNextEngine：同一个 conf_room、同一个机位，实时路径追踪，1 spp/frame。](../../assets/blog/gknextengine-conf-room.webp)
...
4. 这个仓库开出来其实是为了学光追，两年之后那条路径追踪长成了什么样。
5. 七万个小行星背后，GPU-Driven 真正省掉的是哪一段工作。
6. AI Native 是个架构问题，不是接个 ChatGPT。现在我的地图和角色都是 AI 写出来的代码。
...
如果你今天用 UE 或 Unity 做项目，你会觉得这两个引擎已经无所不能。Nanite、Lumen、HDRP、URP、ECS、DOTS，名词多到背不过来。
...
我现在做引擎最深的体会是：在这类通用引擎里，材质和 shader 变体膨胀的一个重要原因，就是同时支持的路径太多。我相信你一定在工作中遇见过这样的情形：

> Compiling 65535 Shaders ...

Shader 变体、构建时间、二进制体积和维护成本都会往上长。每个新 feature 都得考虑"用户没开光追怎么办""跑在 GLES2 设备上怎么办""上个版本写的材质还在用怎么办"。有些回退会落到运行时，更多时候它先变成工程复杂度。以前我把这些都混成一句"GPU 在做判断"，现在看并不准确。
...
第一条是奢侈品，只有 hobby 项目能这么干。但它的回报是，当我决定改一个管线决策的时候，不需要权衡任何人的项目会不会崩。商业引擎当然也能做 Bindless 和光追，只是很难把某一条激进路径变成唯一答案。我这里可以。

我不是说 UE 和 Unity 不好，它们仍然是地球上最好的两个引擎。只是在它们里面，现代路径通常要和历史路径共存；我想看的，是只保留一条现代路径时会长成什么样。

顺带说一句，这个包袱压住的不只是渲染，还有 AI。这事放第六节单独讲。
...
我没做同场景、同 commit 的严格 A/B，所以不能把提升直接归功于 Slang 编译器。可能是编译路径，也可能是翻译过程中顺手消掉了冗余分支。能确认的是迁移没有带来性能回退，至于功劳算谁的，先不硬认。

Slang 还有一个特性值得专门讲，自动微分。原生支持 differentiable shader，给渲染特征的预训练提供了新路径。这里不展开，单独留一篇。

工具链也很现代，可以直接编译到 SPIR-V 和多种文本后端。WebGPU / WGSL 与 Metal 目标已经有了，但官方仍然标成 work in progress。我现在写 shader 已经回不去了，就像用过 C++ 之后没法回去写 C。

如果你今天还在写"vertex shader 和 fragment shader 是两个文件"的 GLSL，去看一眼 Slang，可能会有点 culture shock。
...
但 gkNextEngine 现在走的是一条全 Bindless 的路径。先把边界说清楚：它不是 Vulkan 一次 descriptor set 都不 bind。桌面主线仍然会绑定一个全局纹理 set，Android 的 TLAS 还有传统 binding。真正消失的是每个材质、每个 pass 各自维护的 descriptor 接线；buffer 走地址，纹理走全局数组索引。
...
它不是场景本身，更像目录。shader 再从这些地址找到节点、材质、几何和光照数据。
...
而我不需要把它当成唯一答案之外的另一个分支。这是占了个人项目的便宜。

<p align="center">✦</p>

<h2 align="center">四、这个仓库最开始，是为了学光线追踪</h2>
...
软件追踪那条路径的 shader 几乎逐字相同，差别只在换了两个类型：
...
![玻璃球里的气泡和内反射、粗糙金属、陶瓷，都是这条路径追踪算出来的。取自仓库 README 的录制画面，不作为性能对比。](../../assets/blog/img-pathtracing-luxball.webp)

与之配套的是引擎的渐进式渲染模式，这个我要单独说一下。

编辑器空闲、静态 benchmark 和 agent 截图都会切进这个模式：ReSTIR 退回纯 RIS，SHARC 关掉，时域 upscaler 和帧生成也一起绕过。所有靠"复用上一帧"省下来的东西全部收回，只剩下老老实实一帧一帧累积。它出来的是 ground truth。

这个模式的用处比我预想的大——它是一组随时可以调出来的对照。实时那条路上任何一个复用环节偏了，跟它一比就露馅。

也正是因为手里有这个参考，我才真切感受到 DLSS Ray Reconstruction 的厉害。其他 upscaler 在高粗糙度的反射表面上多少都有亮度损失，只有 DLSS RR 能和 ground truth 对上，看不出偏差。这是我反复肉眼对比的结论，不是受控测量，但看过几个场景之后我不太怀疑它。

至于速度：还是那台 RTX 5070 Ti、720p，路径追踪在内置场景上大致是 2 到 5ms 的帧时间，GI 最重的 GIBootcamp 是 4.950ms，七万实例那个小行星带是 2.089ms。它不是"离线渲染变快了"，是一开始就按实时预算写的东西。

回头看，我当初想学的只是怎么发一条射线。真正吃掉两年的，是射线之外的所有事：样本怎么复用、历史什么时候还可信、没有硬件光追的设备怎么办。
...
数字只代表这台机器和这次构建，不往 4K 或者手机上外推。

2014 年我做过一个迷宫场景，几千个 Quad 就卡到不能玩。那是一条 CPU-driven 的路径，每个物体都对应一段 CPU 侧的提交工作。

现代 GPU-Driven 的核心思路就一句话：让 GPU 自己决定画什么。

gkNextEngine 没把硬件 mesh shader 设成前提，而是用 compute + vertex shader 做了一条 Soft Mesh 路径：compute 逐实例做视锥和遮挡剔除，把可见项压实成一条 primitive stream 和一份 indirect 参数，后面的 graphics pass 只消费这条 stream。
...
省掉的是"按物体绑资源、逐个发 draw"那段 CPU 工作。节点变换、物理和剔除本身仍然随物体数量增长，场景复杂度并没有变成常数。
...
<h2 align="center">六、AI Native 不是接个 ChatGPT：让 AI 写建模代码，而不是直接吐网格</h2>
...
场景、建筑、道具现在不再先做成不可读的二进制网格，而是让 agent 写 OpenSCAD。几何变成了代码：可参数化、能进 git diff、能复用 kit 库。角色也一样，ScadRig 把刚体部件、骨骼层级和关键帧动画写进同一个 `.scad` 文件，`bone_` 前缀的 module 就是骨架，动作是一张纯数据的关键帧表。
...
最近的 NextDayz 是最直接的例子。地图 `riverland_1km.scad` 是 1km²、176×176 格地形，一条东西向公路、一条河加一座桥、8 个据点，全部由地形函数和 kit 组件生成。角色 `nextdayz_survivor.scad` 有 17 根骨骼、31 段动作，覆盖蹲姿、四方向走 / 跑 / 冲刺、翻越、攀爬、举枪瞄准、换弹和开火后坐力。地图、角色、动画和 gameplay 沿着同一套文本资产并行迭代。
...
之前这些是一堆零散脚本，Windows 一套、mac 一套，谁都不敢动。

后来我把它们全部推倒，写了 gnb：一个独立的 Go 命令行工具，不链接引擎本体，第三方依赖只有 CLI 框架和 toml 解析器两个。用户侧不需要懂 Go，仓库根目录的 shim 脚本会自动取预编译二进制。

桌面端的体验是这样的：clone 仓库，`./gnb setup`，`./gnb build`，完了。Android / iOS 也从 gnb 入口走，但仍然需要各自的 SDK、签名和设备环境，不是两条命令凭空把移动工具链变没。

然后这个工具就开始野蛮生长。`gnb loc` 统计代码行数（文末那串 55,279 到 36,492 就是它量的），`gnb dashboard` 后来变成了 Wails 原生窗口，构建、运行、测试、git、todo、文档、LOC，连本地 LLM 聊天都塞进去了。现在还有 `gnb shot`、`gnb validate`、`gnb remote` 和 `gnb llm`。回过神来，gnb 自己已经有 121 个 Go 文件、约 2.1 万行非空代码了。

为什么这个轮子值得造？除了人用着舒服，还有一个第六节的延伸理由：gnb 也是写给 agent 的环境说明书。仓库的 AGENTS.md 里告诉 agent，改了 Engine 层就 `./gnb build gkNextRenderer gkNextUnitTests`，改了单个游戏就只构建那个目标。agent 照做就行，不需要理解五平台的构建细节。
...
但它是开源的。源码挂在 github.com/gameknife/gkNextEngine。Windows / Linux / macOS 可以按上一节两条命令开始；Android / iOS 也是正式支持目标，但要先准备对应 SDK 和签名环境。把"支持五个平台"写成"任意平台两条命令跑通"，说过头了。

它当然不完美，这篇文章的数字已经重算了好几轮。光是最近这段时间，又加了 ScadLibrary 的角色工作台、本地 LLM 驱动的 StudioSim / AirportSim、输入驱动的 agent 自动验证、多 RenderView 和 RemotePlay；与此同时，AMD iGPU 的 descriptor 规则、macOS Vulkan 驱动兼容、多视口的时域历史，也都各自让我补过课。
...
核心层那个数字值得单独说一句，因为它不是一条直线。两年攒到 55,279 行，那轮重构分五个阶段一路搬到 33,924，之后继续收边界，七月中最低到过 31,084；三个星期过去，又长回 36,492。每个阶段挪了多少行都写在 commit message 里，可以自己去翻。

所以"核心保持小、外围可以长"不是砍一次就成立的结论，是需要反复收的边界。至于能不能在一个周末读完，还是看周末有多长，我就不替读者保证了。

如果你恰好在做引擎，或者只是好奇，这是一个可以看的样本。

这一篇装不下的东西还有不少。渲染这边，Bindless 那三个 Vulkan feature 到底怎么咬合、ImGui 怎么接进同一套纹理体系、Visibility Buffer 之后材质系统被迫换的组织方式、实时路径追踪里历史帧什么时候还可信、AmbientCube 的显存账、多相机之后渲染器里为什么不再有"全局状态"。AI 这条线也不止第六节：让 agent 自己截图、回放输入、做机器可判定的断言，一份反射元数据怎么同时喂给编辑器和脚本，还有那次把两万多行搬出核心层的实录。

这些都会慢慢单独成文，但顺序我不排死——哪一篇的证据先凑齐，就先写哪一篇。有想先看的，评论告诉我。
